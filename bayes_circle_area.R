...
for(which_cond in c("triangle","horizontal")){
  for(which_model in c("sigma_constant","sigma_varying")){
    ...
    dat_all <- here("data","circle_area","aggregated") %>%
      dir_ls(regexp = ifelse(outliers_removed,"no_outliers","with_outliers")) %>%
      read_csv() %>%
    ...
    wt <- wd <- as.integer(dat_all_clean$set=="a-b-db") # if a-b-db, t & d are wide
    wc <- as.integer(dat_all_clean$set=="a-b-da") # if a-b-da, c is wide
