function bar_locs = RDCL_BarDisplay(bar_options)

% Displays a bar graph
%
%  screen_ptr = a pointer to the psychtoolbox screen
%   bar_options = a struct with the arguments to make the bars
%     .dim_1_level: 0-1
%     .dim_2_level: 0-1
%     .dim_3_level: 0-1
%     .dim_4_level: 0-1
...
% Example:
%
%	bar_options.dim_1_level = .5;
%	bar_options.dim_2_level = .05;
%	bar_options.dim_3_level = 1;
%	bar_options.dim_4_level = .75;
%	bar_options.axis_size = .7;
% 	bar_options.axis_color = [255 255 255];
% 	bar_options.labels = {'Label 1', 'Label 2', 'Label 3', 'Label 4'};
%	bar_options.title = 'Runner = 338';
%
%	RDCL_BarDisplay(bar_options)
%
% Andrew Cohen
% 1/14/11
...
% Get options
dim_1_level = max(min(bar_options.dim_1_level, 1), 0);
dim_2_level = max(min(bar_options.dim_2_level, 1), 0);
dim_3_level = max(min(bar_options.dim_3_level, 1), 0);
dim_4_level = max(min(bar_options.dim_4_level, 1), 0);
axis_size = max(min(bar_options.axis_size, 1), 0);
axis_color = bar_options.axis_color;

dim_1_label = bar_options.labels{1};
dim_2_label = bar_options.labels{2};
dim_3_label = bar_options.labels{3};
dim_4_label = bar_options.labels{4};

title = bar_options.title;
...
tmp_horiz_axis_y = tmp_vert_axis_end_y;
...
for d = 1:4
    ...
    bar_y_start = round(horiz_axis(2) - dim_level*tmp_axis_v_length);
...
for d = 1:4
    ...
    Screen('DrawText', screen_ptr, curr_label, ...
        ...
        round(curr_bar(4) + text_label_buffer*bounds(4)), ...
        axis_color);
...
Screen('DrawText', screen_ptr, title, ...
    ...
    round(horiz_axis(2) + bounds(4) + 3*text_label_buffer*bounds(4)), ...
    axis_color);
