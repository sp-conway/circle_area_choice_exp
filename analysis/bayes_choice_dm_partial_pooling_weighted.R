...
# control settings
which_model <- "bayes_choice_dm_partial_pooling_weighted_1"
...
for(which_cond in c("triangle","horizontal")){
  ...
  fit <- m$sample(data=stan_data,
                  ...
                  output_basename=paste0("bayes_choice_",today()))
  draws <- fit$draws()
  ...
  mcmc_trace(draws,regex_pars = "alpha_g")
  ggsave(filename=path(results_dir,"alpha_g_trace.jpeg"),width=8,height=8)
  diagnostics <- fit$diagnostic_summary()
  save(diagnostics,file=path(results_dir,"diagnostics.RData"))
  
  mcmc_dens(draws,"lp__")
  ggsave(filename=path(results_dir,"lp__dens.jpeg"),width=8,height=8)
  mcmc_dens(draws,regex_pars = "p_m",facet_args = list(scales="fixed"))
  ggsave(filename=path(results_dir,"p_m_dens.jpeg"),width=8,height=8)
  mcmc_dens(draws,regex_pars = "alpha_g",facet_args = list(scales="fixed"))
  ggsave(filename=path(results_dir,"alpha_g_dens.jpeg"),width=8,height=8)
  
  mcmc_hist(draws,"lp__")
  ggsave(filename=path(results_dir,"lp__hist.jpeg"),width=8,height=8)
  mcmc_hist(draws,regex_pars = "p_m",facet_args = list(scales="fixed"))
  ggsave(filename=path(results_dir,"p_m_hist.jpeg"),width=8,height=8)
  mcmc_hist(draws,regex_pars = "alpha_g",facet_args = list(scales="fixed"))
  ggsave(filename=path(results_dir,"alpha_g_hist.jpeg"),width=8,height=8)
  ...
  #   ggplot(aes(reorder(participant,-mean),mean))+
  #   geom_point(alpha=.5,shape="X")+
  #   geom_errorbar(aes(ymin=`2.5%`,ymax=`97.5%`),alpha=.5)+
  #   scale_y_continuous(limits=c(0,1))+
  #   labs(x="participant",y=TeX("$\\omega$ estimates"))+
  #   ggthemes::theme_few()+
  #   theme(axis.text.x = element_blank(),
  #         axis.ticks.x=element_blank(),
  #         text = element_text(size=18))
  # ggsave(filename=path(results_dir,"omega_estimates.jpeg"),width=5,height=5)
