# fitting bayesian hierarchical model to circle data (attraction/repulsion trials)
# I analyze fitting results in a separate file - bayes_circle_area_analyze_model.R
...
library(bayesplot)
library(cmdstanr) 
set_cmdstan_path(here("cmdstan-2.36.0")) # have to set cmdstan path - varies based on user's local directory
...
for(which_cond in c("triangle","horizontal")){
  ...
  # if target_effect:
  # mu_ijk = (b0+s0_i) + (bw*w_ijk) + (bdiag2*diag2_ij) +
  #           (btar*tar[i]) # don't actually include this as a variable, do it all in stan
  #           (bdiag3*diag3_ij) +
  #           (bd5*distance5_ij) +
  #           (bd9*distance9_ij) +
  #           (bd14*distance14_ij) +
  ...
  # figure out how many per subject
  # note that because we removed outliers, not all subjects have equal numbers of trials
