...
run_m_fe <- function(data, f){
  if(!file_exists(f)){
    m <- lm(la_c~w+diag2+diag3+d5+d9+d14+d2d+d5d+d9d+d14d,data=data)
    save(m, file=f)
  }else{
    # browser()
    load(f)
...
run_m_me <- function(data, f){
  if(!file_exists(f)){
    m <- lmer(la_c~w+diag2+diag3+d5+d9+d14+d2d+d5d+d9d+d14d+(1|sub_n),data=data)
    save(m, file=f)
    # browser()
    load(f)
    # plot(f)
...
for(cond in conds){
  mydir <- here(glue("lmer_no_cor_{cond}{tmp}",tmp=ifelse(sean_data,"_sean","")))
  dir_create(mydir)
  d <- load_data(dataf, cond)
  ffe <- path(mydir,glue("fit_fe.RData"))
  fme <- path(mydir,glue("fit_me.RData"))
  run_m_fe(d,ffe)
  run_m_me(d, fme)
}
